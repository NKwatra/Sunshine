export default parseData = data => {
  const weatherForcast = data.map(
    ({ max_temp, min_temp, valid_date, weather: { description } }, index) => {
      ...
      if (index === 0)
        return `Today, ${month} ${date} - ${description} - ${max_temp}℃ / ${min_temp}℃`;
      else if (index === 1)
        return `Tomorrow - ${description} - ${max_temp}℃ / ${min_temp}℃`;
      else if (index >= 2 && index <= 6)
        return `${day} - ${description} - ${max_temp}℃ / ${min_temp}℃`;
      else
        return `${day}, ${month} ${date} - ${description} - ${max_temp}℃ / ${min_temp}℃`;
    }
  );
